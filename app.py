...
.centered-title { text-align: center; font-size: 2.2em; font-weight: bold; margin-bottom: 0.2em; display: flex; align-items: center; justify-content: center; }
.centered-title img { margin-right: 10px; }
.centered-subtitle { text-align: center; font-size: 1.1em; color: gray; margin-bottom: 2em; }
.stTextArea textarea { border: 2px solid #4CAF50; border-radius: 12px; padding: 12px; font-size: 16px; }
.highlight-box { background-color: #fff9c4; padding: 10px; border-radius: 8px; }
</style>
...
st.markdown('<p class="centered-subtitle"><b>Gender-Fair Language Detection Tool</b></p>', unsafe_allow_html=True)
...
with col1:
    ...
    with b3:
        st.button("🔎 Scan", on_click=lambda: process_text(st.session_state.input_text_widget))
